...
import {
  generateDatePickerRestrictions,
  generateTimePickerRestrictions,
} from '@/utils/getDatepickerRestrictions';
...
import PlanAutoSchedule from './planAutoScheduleForm';
import InputFileUpload from './fileUpload';
// import MaxHeightTextarea from './textArea';
import GenericFilePicker from './genericFilePicker';
import ColumnsMatchSelector from './columnsMatchSelector';
...
// const PlanAutoSchedule = dynamic(() => import('./planAutoScheduleForm'), { ssr: false });
// const InputFileUpload = dynamic(() => import('./fileUpload'), { ssr: false });
const MaxHeightTextarea = dynamic(() => import('./textArea'), { ssr: false });
const CountrySelect = dynamic(() => import('./countrySelect'), { ssr: false });
// const GenericFilePicker = dynamic(() => import('./genericFilePicker'), { ssr: false });
// const ColumnsMatchSelector = dynamic(() => import('./columnsMatchSelector'), { ssr: false });
        ...
        fields.map((item) => {
          return (
            <Grid item xs={12} md={item.md || 12} key={item.name} justifyContent="center">
                  ...
                  <Box style={{ marginTop: "8px", marginBottom: "4px" }}>
                    <MobileDateTimePicker
                      ...
                      shouldDisableDate={(date) => {
                        if (!item.restrictions) return;
                        const days = generateDatePickerRestrictions(item.restrictions);
                        const day = date.day();
                        return !days.includes(day)
                      }}
                      shouldDisableTime={(value, view) => {
                        if (!item.restrictions) return;
                        return !generateTimePickerRestrictions(value, item.restrictions);
                      }}
                    />
                  </Box>
                )
                ...
                item.type === "staticdatetimepicker" && (
                  <Box style={{ display: "flex", justifyContent: "center" }}>
                    <StaticDateTimePicker
                      ...
                      minDate={item.minDate || null}
                      shouldDisableDate={(date) => {
                        if (!item.restrictions || item.restrictions.isOnlyStartTime === null) return;
                        return generateDatePickerRestrictions(date, item.restrictions.planRestriction);
                      }}
                      shouldDisableTime={(value, view) => {
                        if (!item.restrictions || item.restrictions.isOnlyStartTime === null) return;
                        return generateTimePickerRestrictions(value, item.restrictions, userTimezone);
                      }}
              ...
              {
                item.type === "planAutoSchedule" && (
                  <PlanAutoSchedule
                    ref={refPlanAutoSchedule}
                    presetConfig={item.default}
                    action={item.action}
                  />
                )
              }
              ...
              {
                item.type === "columnsMatchSelector" && <ColumnsMatchSelector
                  ref={refColumnsMatchSelector}
                  fieldName={item.name}
                  optionsLeft={item.optionsLeft}
                  optionsRight={item.optionsRight}
                  linkingField={item.linkingField}
                  descriptionField={item.descriptionField}
                />
              }
              {
                item.type === "genericfilepicker" && (
                  <GenericFilePicker ref={refGenericFile} />
                )
              }
              {
                item.type === "textArea" && (
